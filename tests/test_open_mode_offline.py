...
def test_static_research_pages_public_openapi_and_mcp(monkeypatch):
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        token = sandbox_key["token"]
        assert token.startswith("13flow_sandbox_")
        assert c.get("/api/sandbox/v1/status").status_code == 401
        ...
        assert "Changes between snapshots" in pro_workspace_page
